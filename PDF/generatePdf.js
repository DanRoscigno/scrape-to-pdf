...
function getUrls(url) {
    ...
    // which needs to be modified to the `docusaurus311` service 
    // in the Docker compose environment
    let docusaurusUrl = 
        url.replace("localhost", "docusaurus311").replace("0.0.0.0", "docusaurus311");
...
async function callGotenberg(docusaurusUrl, fileName) {
    ...
    try { 
        ...
        const response = await axios.post(
          "http://gotenbergAPI:3000/forms/chromium/convert/url",
          form,
          {
